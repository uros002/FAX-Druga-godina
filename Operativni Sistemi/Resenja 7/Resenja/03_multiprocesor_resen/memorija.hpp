...
private:
    ...
    vector<shared_ptr<Pristup>> pristupi;  // Drugi vektor je vektor pokazivača na strukture koje beleže pristupe datoj reči
...
public:
    Memorija(Dijagnostika& d, int bajtova) : dijagnostika(d), mem(bajtova) {
        ...
        for (int i = 0; i < reci; i++) {                                // Primer - 10 bajta = 3 reči. Generiše se 3 instance strukture Pristup.
            pristupi.push_back(make_shared<Pristup>());
        }
    }
