...
void testirajSve() {
  ...
  uniform_int_distribution<int> raspodela_broja_stranica(1, 4);
  ...
  for (int i = 0; i < UKUPNO_PROCESA; ++i){
      ...
      t_proc[i] = thread(proces, ref(rm), i+1, raspodela_broja_stranica(dr), static_cast<Tip_procesa::tip_procesa>(i % 2));
   }
