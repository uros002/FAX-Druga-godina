...
void radnik(UlazniPodaci<pair<double, double>> &ulaz, IzlazniPodaci<vector<double>> &izlaz) {
    ...
    pair<double, double> par_granica;  // promenljiva u koju ce metoda UlazniPodaci::preuzmi() smestiti preuzet element iz kolekcije 
    default_random_engine eng;  // svaki radnik pravi svoj generator brojeva ...
    eng.seed(steady_clock::now().time_since_epoch().count());  // ... i posebno ga priprema za generisanje
    ...
    while (ulaz.preuzmi(par_granica)) {  // metoda UlazniPodaci::preuzmi() vraca true samo kada uspesno preuzme podatak; u telo petlje ide logika obrade podataka
        uniform_real_distribution<double> distribucija(par_granica.first, par_granica.second);  // inicijalizacija objekta distribucije potrebnog za generisanje brojeva
...
void citacf(string ime_ulazne_dat, UlazniPodaci<pair<double, double>> &ulaz) {
    ...
    if(ulazna_dat.is_open()) {
		while(getline(ulazna_dat, red_datoteke)) {  // citanje datoteke red po red
			...
			ulaz.dodaj({donja_granica, gornja_granica});  // slanje strukture sa dva polja double koristeci se {}, sto je dozvoljeno jer ova struktura odgovara definiciji pair<double, double>
		}
