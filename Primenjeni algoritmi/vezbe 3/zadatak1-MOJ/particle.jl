...
# 4*x^2 - 6*x - 3*y^3 + 0.5*y + 3*z + 8*w - 6.1*u +2*v - 10
...
function generateParticle(numberOfOperands,minRang,maxRang)
        particle = Particle(rand(minRang:0.1:maxRang,numberOfOperands), 0, fill(0.0, numberOfOperands),fill(0.0,numberOfOperands),0)
...
function printParticle(particle)
        ...
        u = particle.values[5]
        v = particle.values[6]
        print("4*$(round.(x;digits = 4))^2 - 6*$(round.(x;digits = 4)) - 3*$(round.(y;digits = 4))^3 
...
function updateVelocity!(particle,globalBestParticle,maxVelocity)
        ...
        for i in 1:length(particle.values)
                particle.velocity[i] += rp*2.0*(particle.bestValues[i] - particle.values[i]) + 
                                        rg*2.0*(globalBestParticle.values[i] - particle.values[i])
