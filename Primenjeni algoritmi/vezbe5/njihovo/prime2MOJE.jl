...
for i in 1:length(predvidjenaCena)
        println("X$i: $(dataTest.proizvodjac[i]) $(dataTest.model[i]) $(dataTest.kubikaza[i])
        $(dataTest.godiste[i]) CENA : $(dataTest.cena[i]) PROCENA: $(predvidjenaCena[i])")
end
...
println()
println("Spisak svih gresaka pri testiranju je: $(round.(abs.(errorsTest)))")
#Racunanje i ispis proseka gresaka
